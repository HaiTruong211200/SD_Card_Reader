...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  bool sd_direct_test = false;
  sd_direct_test = SD_Init();
  bool mount_result = false;
  bool write_test_result = false;
  bool read_test_result = false;
  bool compare_test_result = false;
  bool append_result = false;
  mount_result = Storage_Mount();

  if (mount_result)
  {
	write_test_result = Storage_CreateAndWriteTest();
  }

  if (write_test_result)
  {
	read_test_result = Storage_ReadTest();
  }

  if (read_test_result)
  {
	compare_test_result = Storage_CompareTest();
  }
  bool list_result = false;
  list_result = Storage_ListRoot();

  bool delete_result = false;

  if (list_result)
  {
	delete_result = Storage_DeleteTest();
  }
  list_result = Storage_ListRoot();
  bool storage_info_result = false;

  storage_info_result = Storage_GetInfo();
