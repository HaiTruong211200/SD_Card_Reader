...
/* Definitions for defaultTask — stack lớn hơn để chạy luôn Work4 UART */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  /*
   * LTDC mặc định FBStartAdress=0 → màn trắng.
   * Gắn FB vào SDRAM, tô đen, bật LCD trước khi TouchGFX task chạy.
   */
  {
  ...
  UartPc_Init(&huart1);
  /* Không tạo task UART riêng — tránh hết heap FreeRTOS làm GUI fail */
...
static void MX_LTDC_Init(void)
{
  ...
  /* LCD Init */
  LcdDrv->Init();
  /* Không DisplayOff ở đây — để tránh màn trắng kéo dài trước khi TouchGFX bật lại */
...
{
  /* USER CODE BEGIN 5 */
  /* Chờ TouchGFX lên hình rồi mới nhận lệnh UART (Work 4) */
  osDelay(2000);
  UartPc_Task(argument);
  /* USER CODE END 5 */
}
